...
""" compute_discriminator_loss(discriminator, visual_state, action, audio_state, discriminator_weighted=False):
    expected_audio_dim = discriminator.observation_shape_sound[0]
    device = visual_state.device
    ...
    ""for i in range(5):
        print(f"audio_state[{i}][:10]:", audio_state[i][:10])
        print(f"audio_state[{i}].std():", audio_state[i].std().item())
    print("Nombre de sons uniques dans le batch :", torch.unique(audio_state, dim=0).shape[0])
    print("Nombre de sons uniques dans le batch (après permutation) :", torch.unique(audio_state_false, dim=0).shape[0])""
