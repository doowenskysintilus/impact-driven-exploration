...
class MiniGridEnv(gym.Env):
    ...
    def __init__(
        ...
        sound_dim: int = 10, # Number of sound channels
    ):
        # Initialize mission
        ...
        # Initialisez les attributs audio
        self.sound_dim = sound_dim
        self.current_sound = np.zeros(sound_dim)
        ...
        def _generate_sound_observation(self):
            """Génère une observation sonore basée sur l'état actuel"""
            sound = np.zeros(self.sound_dim)
            
            # Exemple simple: le son dépend de la position de l'agent
            sound[0] = self.agent_pos[0] / self.width  # Normalisé entre 0 et 1
            sound[1] = self.agent_pos[1] / self.height
            
            # Ajoutez d'autres caractéristiques sonores selon votre besoin
            if self.carrying:
                sound[2] = 1.0  # Indicateur de port d'objet
    ...
    def gen_obs(self):
        ...
        sound =  self._generate_sound_observation()
